...
bool same_next_day(int prev_day, int prev_month, int prev_year, int day,
	...
	if (isLeap(year) and prev_day == 28 and prev_month == 2) {
		...
		if (prev_day == 29 and prev_month == 2) prev_day--;
		prev_day = (prev_day + 1) % months[prev_month - 1];
		if (!prev_day) prev_day = months[prev_month - 1];
		if (prev_day == 1) {
		...
		if (prev_day == prev_month and prev_month == 1) prev_year++;
	}
...
int main() {
	...
	while (cin >> t and t) {
		...
		while (t--) {
			...
			if (same_next_day(prev_day, prev_month, prev_year, day,
					  month, year)) {
				cnt++;
